defmodule KoreanSentenceAnalyser.ETS.DictFile do
  ...
  def init_dict_from_file(file) do
    ...
    File.stream!(file)
    |> Stream.map(fn x -> insert(x, file) end)
    |> Stream.run()
  end

  @doc """
  Insert a word into the ETS dictionary
  """
  def insert(word, file) do
    word = String.replace(word, "\n", "")

    :ets.insert(String.to_atom(file), {word})
  end
  ...
  def find(word, file) do
    case :ets.select(String.to_atom(file), [{{word}, [], [:"$_"]}]) do
      [{word}] -> word
      _ -> nil
    end
  end
