defmodule KoreanSentenceAnalyser.Helpers.KoreanUnicode do
  ...
  defp split(<<decimal_point :: utf8, rest :: binary>>, character, acc) when decimal_point > @jamo_start_location_in_unicode and decimal_point < @start_location_in_unicode do
  ...
  def starts_with?(word, match) do
    deduct_jamo_start(get_unicode_decimal_value(match)) == Float.floor(
      deduct_start(get_unicode_decimal_value(String.first(word))) / @characters_per_initial
    )
  end
